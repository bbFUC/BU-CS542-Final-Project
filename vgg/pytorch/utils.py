...
class Trainer(object):
    ...
    def _iteration(self, data_loader, epoch, is_train=True):
        ...
        for data, target in tqdm(data_loader, ncols=80):
            ...
            accuracyK_count = 0   # used to count the topK accuracy
        ...
        if is_train:
            ...
            self.writer2.add_scalars('epoch_loss', {"valid_loss":sum(loop_loss)}, epoch)
            self.writer2.add_scalars('epoch_accuracy&accuracyK', {"valid_accuracy":sum(accuracy) / len(data_loader.dataset), "valid_accuracyK":sum(accuracyK) / len(data_loader.dataset)}, epoch)
    ...
    def loop(self, epochs, train_data, test_data, current_epoch, scheduler=None):
