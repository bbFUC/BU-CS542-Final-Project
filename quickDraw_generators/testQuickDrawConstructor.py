...
def drawByNdjson(raw_dataPath, randomSample=0, start=0, end=10, scale_length=2.56, scale_width=2.56, savingPath="./"):
    f = open(raw_dataPath)
    setting = json.load(f)
    print(len(setting))
    ...
    if randomSample == 0:
        ...
        for j in range(start, end):
            ...
            plt.close()
            print(str(count) + ".png" + " has been stored!")
            count += 1
        ...
        for j in resultList:
            ...
            #将图片存起来
            plt.savefig(savingPath + "/%s.png"%num)
            plt.close()
            print(str(count) + ".png" + " has been stored!")
            count += 1
...
if __name__ == '__main__':
    ...
    end = 0     # the index of the last draw to be generated, if end=0 then iterate all
    ...
    #files paths
    savingPath = "/Users/liweixi/Downloads/test_quickdraw_visualize/"    # path of saving quickdraws
    categoryPath = '/Users/liweixi/Downloads/quickdraw_categories.txt'  # path of the quickdraws category
    datasetPath = '/Users/liweixi/Downloads/test_quickDraws.json'    # path of the quickdraw raw datasets
    ...
    if target_category==[]:
                raw_dataPath = datasetPath
                category_savingPath = savingPath
                mkdir(category_savingPath)
                drawByNdjson(raw_dataPath, randomSample, start, end, scale_length, scale_width, category_savingPath)
